...
public:
	...
	void test_print_textTruecolor() {
		...
		// 19 for SDL_ttf <= 2.0.14, 18 for SDL_ttf >= 2.0.15
		// https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=923407
		TS_ASSERT_EQUALS(img->h, 19);
		TS_ASSERT_EQUALS(getIndexAt(img, 0, 0), 0);
		TS_ASSERT_EQUALS(getIndexAt(img, 1, 4), 1);
		ce = {255, 255, 2, 255};
		TS_ASSERT_SAME_DATA(&img->format->palette->colors[1], &ce,
							sizeof(SDL_Color));
		ce = {255 - 255, 255 - 255, 255 - 2, 0};
		TS_ASSERT_SAME_DATA(&img->format->palette->colors[0], &ce,
							sizeof(SDL_Color));
	...
	void test_print_text_wrap_getcmdsTruecolor() {
		...
		r = {0, 0, 150, 100};
		TS_ASSERT_EQUALS(
				print_text_wrap_getcmds("toto", &r, 1, 0, FONT_DIALOG, &cmds),
				19);
