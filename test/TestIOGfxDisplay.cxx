...
public:
	...
	void ctest_blit() {
		...
		if (display->truecolor) {
			...
			pixels[5] = 0;
			pixels[img->pitch] = 255;
			pixels[img->pitch + 1] = 0;
			pixels[img->pitch + 2] = 0;
			pixels[img->pitch + 3] = 255;
			pixels[img->pitch + 4] = 0;
			pixels[img->pitch + 5] = 0;
	...
	void test01SplashGL2Truecolor() {
		openDisplay(true, true,
					0); // 0 <= off-screen rendering may have bugs >(
		ctestSplash();
		closeDisplay();
	}
