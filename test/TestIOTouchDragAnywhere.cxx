...
void pushEvent(IOTouchDragAnywhere* conv, SDL_EventType type, SDL_TouchID tid,
			SDL_FingerID fid, float x, float y) {
	SDL_Event ev;
	ev.type = type;
...
public:
	...
	void testTransform() {
		...
		// Set center of virtual joystick
		{
			...
			SDL_Event getev;
			TS_ASSERT_EQUALS(0,
							SDL_PeepEvents(&getev, 1, SDL_GETEVENT,
		...
		// First motion above the center triggers KEYUP
		{
			...
			TS_ASSERT_EQUALS(0,
							SDL_PeepEvents(&getev, 1, SDL_GETEVENT, SDL_KEYUP,
											SDL_KEYDOWN));
		}
