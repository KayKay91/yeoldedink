...
void IOGfxDisplay::close() {
	if (window) {
		#ifndef DINKEDIT
		if (mode > 2)
		ImGui::SaveIniSettingsToDisk(paths_pkgdatafile("imgui.ini"));
		#endif
...
void IOGfxDisplay::logDisplayInfo() {
	log_info("🖌️ Truecolor mode: %s", truecolor ? "on" : "off");

	Uint32 Rmask = 0, Gmask = 0, Bmask = 0, Amask = 0;
	int bpp = 0;
	SDL_PixelFormatEnumToMasks(getFormat(), &bpp, &Rmask, &Gmask, &Bmask,
							&Amask);
	log_info("🖌️ SW buffer format: %s %d-bit R=0x%08x G=0x%08x B=0x%08x A=0x%08x",
...
void IOGfxDisplay::logWindowInfo() {
	log_info("🖌️ Video driver: %s", SDL_GetCurrentVideoDriver());
	log_info("🖌️ Video fall-back surface (unused): %s",
...
void IOGfxDisplay::toggleFullScreen() {
	SDL_SetWindowFullscreen(window,
							(SDL_GetWindowFlags(window) & SDL_WINDOW_FULLSCREEN) ? 0 : SDL_WINDOW_FULLSCREEN_DESKTOP);
	// Note: software renderer is buggy in SDL 2.0.2: it doesn't resize the surface
}

void IOGfxDisplay::toggleBorderless() {
	SDL_SetWindowBordered(window, (SDL_GetWindowFlags(window) & SDL_WINDOW_BORDERLESS) ? SDL_TRUE : SDL_FALSE);
}
