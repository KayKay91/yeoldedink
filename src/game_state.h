...
struct player_info {
	int minutes;
	//ye: add one to compare to original game
	struct item_struct mitem[NB_MITEMS + 1];
	...
	char palette[50];
	//ye: only save the first 41 tilescreens
	struct player_info_tile tile[GFX_TILES_NB_SETS];
	...
	unsigned int
			push_timer; /* time the player started maintaining the direction */
...
extern unsigned int
		dink_base_push; /* base Dink push sequence, cf. set_dink_base_push */

/* Engine variables directly mapped with DinkC variables */
extern int *pvision, *plife, *presult, *pspeed, *ptiming, *plifemax, *pexp,
		*pstrength, *pcur_weapon, *pcur_magic, *pdefense, *pgold, *pmagic,
		*plevel, *plast_text, *pplayer_map, *pmagic_level, *pupdate_status,
		*pmissile_target, *penemy_sprite, *pmagic_cost, *pmissle_source;
