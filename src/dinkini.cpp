...
//To toggle scripting engine availability from dink.ini
#ifndef DINKEDIT
#include "dinklua.h"
#include "dinkc.h"
#include "sfx.h"
...
void pre_figure_out(char* line, bool playmidi) {
	...
	#ifndef DINKEDIT
	//Add a thing for Lua
	if (compare(command, "dinklua_enable") || compare(command, "dinklua_enabled") || compare(command, "dinklua_on")) {
		log_info("🌖 Switching on DinkLua from Dink.ini");
		dinklua_enabled = 1;
	}

	//And another for DinkC
	if (compare(command, "dinkc_off")) {
		log_info("🔤 DinkC switched off by dink.ini");
		dinkc_enabled = 0;
	}

	if (compare(command, "sprite_enable_smart_movement")) {
		//switch on seseler's movement tweaks
		dbg.monsteredgecol = true;
	}

	if (compare(command, "sprite_enable_large_talkbox")) {
		debug_talkboxexp = true;
	}

	if (compare(command, "sprite_pig_explosion_seq")) {
		//pig explosion sequence
		gfx_pig_death_seq = atol(ev[1]);
	}

	if (compare(command, "sprite_duck_explosion_seq")) {
		//ducks
		gfx_duck_death_seq = atol(ev[1]);
	}

	//audio
	if (compare(command, "sfx_bass_boost")) {
		sfx_set_bassboost(atof(ev[1]));
		log_info("📜 Bass boost of %.2f applied", atof(ev[1]));
	}

	//graphics
	if (compare(command, "gfx_diag_idle_enable")) {
		log_info("📜 Diagonal idle enabled for player");
		debug_idledirs = true;
	}

	if (compare(command, "gfx_alttext_enable")) {
		log_info("📜 Alternate text display enabled");
		debug_alttext = true;
	}

	if (compare(command, "gfx_alttext_disable")) {
		log_info("📜 Alternate text display off");
		debug_alttext = false;
		debug_alttextlock = true;
	}

	if (compare(command, "gfx_full_fade_enable")) {
		log_info("🌑 Switching on full fades");
		debug_fullfade = true;
	}

	if (compare(command, "gfx_tiles_water_start")) {
		tiles_water_start_sheet = atoi(ev[1]);
	}

	if (compare(command, "gfx_tiles_water_length")) {
		tiles_water_flip_range = atoi(ev[1]);
	}

	if (compare(command, "gfx_tiles_fire_length")) {
		tiles_fire_flip_range = atoi(ev[1]);
	}

	if (compare(command, "gfx_tiles_fire_start")) {
		tiles_fire_start_sheet = atoi(ev[1]);
	}

	if (compare(command, "gfx_tiles_anim_off")) {
		log_info("📜 Tile animations off");
		debug_tileanims = false;
	}

	if (compare(command, "input_controller_set_key")) {
		debug_authorkey = atoi(ev[1]);
	}

	if (compare(command, "scripts_enable_divequals")) {
		debug_divequals = true;
	}

	if (compare(command, "scripts_enable_multiscript")) {
		debug_scriptkill = true;
	}
	#endif
...
void figure_out(char* line) {
	...
	// LOAD_SEQUENCE_NOW  path  seq  NOTANIM
	// LOAD_SEQUENCE_NOW  path  seq  speed  offsetx offsety  hard.left hard.top hard.right hard.bottom
	if (compare(command, "LOAD_SEQUENCE_NOW") || compare(command, "LOAD_SEQUENCE")) {
		rect hardbox = {0};

		int myseq = atol(ev[2]);
		//ye: skip if all is same
		if (dbg.gfxspeed && seq[myseq].len > 0 && compare(line, seq[myseq].ini)) {
			//log_error("old is %s and new is %s", seq[myseq].ini, line);
			return;
		}
		seq[myseq].is_active = 1;
		seq_set_ini(myseq, line);
		...
		log_debug("🎞️ Set frame.  %d %d %d", myseq, myframe, special);
	}
