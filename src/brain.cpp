...
//sttings and fixess
bool debug_drawdam = true;
bool debug_nohitfix = true;
...
void add_kill_sprite(int h) {
	...
	{
		log_error("☠️ Can't make a death sprite for dir %d!", base + dir);
	}
	//Exploding blood?
...
/*
int get_distance_and_dir_smart(int h, int h1, int* dir) {
	//A* pathfinding, unfinished
	//TODO: optimise somewhat, or get rid of
	...
	for (y1 = 0; y1 < 400; y1++)
		...
		//h1 is the sprite that is being targeted
	AStar::Coord2D startPos(spr[h1].x, spr[h1].y);
	AStar::Coord2D endPos(spr[h].x, spr[h].y);
	mypathfinder.setWorldData(600, 399, myhardmap.data());
	auto path = mypathfinder.findPath(startPos, endPos);
	//distance is amount of elements
	log_debug("distance is %d", path.size());
	//set direction based upon next element, accounting for difference in y co-ordinate systems
	if (path.size() > 0) {
		int pathy = (400 - path[1].y);
		log_debug("x1 is %d, y1 is %d, pathy is %d", path[1].x, path[1].y, pathy);
		//right
		if (path[1].x > spr[h].x) {
			if (pathy > spr[h].y)
				*dir = 3;
		...
		log_debug("dir is %d", *dir);
	}
	//assume it couldn't find a path
	if (path.size() > 0)
		return path.size();
	else
		return 5000;
...
int get_distance_and_dir(int h, int h1, int* dir) {
	...
	//if (smooth_follow == 2)
	//	return get_distance_and_dir_smart(h, h1, dir);
	else
...
void process_follow(int h) {
	if (spr[h].follow > MAX_SPRITES_AT_ONCE) {
...
void process_move(int h) {
	...
	if ((spr[h].move_dir == 4) | (spr[h].move_dir == 1) |
		(spr[h].move_dir == 7)) {
	...
	if ((spr[h].move_dir == 6) | (spr[h].move_dir == 9) |
		(spr[h].move_dir == 3)) {
		if (spr[h].x >= spr[h].move_num) {
			//done moving
			done_moving(h);
	...
	if (k > 1)
		if (spr[k].brain != 9)
			if (spr[k].brain != 10) {
				...
				if (hspeed > 49) {
					log_debug("🏎️ Speed was %d", hspeed);
					spr[k].speed = 49;
				} else
					spr[k].speed = hspeed;
			}
...
int autoreverse_diag(int j) {
	...
	log_debug("🚶 Auto Reverse Diag was sent a dir %d sprite, base %d walk.",
