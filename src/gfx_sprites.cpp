...
//Shadow patch
bool check_solid_pixel(SDL_Surface *surf, int x, int y) {
	Uint32 alph, r, g, b;
	Uint32 *px;
	SDL_PixelFormat *fmt;
	fmt = surf->format;
	px = (Uint32*)surf->pixels;
	alph = px[(y * surf->w) + x] & fmt->Amask;
	alph = alph >> fmt->Ashift;
	alph = alph << fmt->Aloss;
	r = px[(y * surf->w) + x] & fmt->Rmask;
	r = r >> fmt->Rshift;
	r = r << fmt->Rloss;
	g = px[(y * surf->w) + x] & fmt->Gmask;
	g = g >> fmt->Gshift;
	g = g << fmt->Gloss;
	b = px[(y * surf->w) + x] & fmt->Bmask;
	b = b >> fmt->Bshift;
	b = b << fmt->Bloss;

	if (alph == 255 && r == 0 && g == 0 && b == 0)
	return true;
	else
	return false;
}

bool check_alpha_pixel(SDL_Surface *surf, int x, int y) {
	Uint32 alph, r, g, b;
	Uint32 *px;
	SDL_PixelFormat *fmt;
	fmt = surf->format;
	...
	if (alph == (255 / 2) && r == (255 / 2) && g == (255 / 2) && b == (255 / 2))
	return true;
	else
	return false;
}
...
void load_sprite_pak(char seq_path_prefix[100], int seq_no, int delay,
	...
	for (oo = 1; oo <= MAX_FRAMES_PER_SEQUENCE; oo++) {
		...
		if (myslot >= MAX_SPRITES) {
			log_error("🧚‍♀️ No sprite slot available! Index %d out of %d.", myslot,
					MAX_SPRITES);
			break;
		}
...
intent to support anything else than 8bit BMPs. */
		...
		during fade_down()/() (255/white pixels can't be
		...
		if (!truecolor)
			SDL_SetPaletteColors(surf->format->palette,	ImageLoader::blitFormat->format->palette->colors, 0, 256);
...
void load_sprites(char seq_path_prefix[100], int seq_no, int delay, int xoffset,
	...
	for (oo = 1; oo <= MAX_FRAMES_PER_ABUSED_SEQUENCE; oo++) {
		...
		if (oo < 10)
			...
			leading_zero = "";
		//Yeolde: changed this so it can support PNGs without having to do that renaming crap
		...
		// Let's put a half-assed reimplementation of redink1's shadow patch here
		//TODO: actually let's put a thing here to convert white to transparent
		SDL_PixelFormat *fmt;
		Uint32 alph, r, g, b;
		SDL_Surface *converted = NULL;
		if (surf->h > 1 && surf->w > 1 && truecolor && dbg.debug_goodshadows)
		{
			converted = SDL_ConvertSurfaceFormat(surf, SDL_PIXELFORMAT_ARGB32, 0);
			fmt = converted->format;
			SDL_LockSurface(converted);
			Uint32 *px = (Uint32 *)converted->pixels;
			for (int y = 0; y < converted->h; y++)
			{
				for (int x = 0; x < converted->w; x++)
				{
					// See if we have a solid pixel
					// Get our individual pixel and its colours
					alph = px[(y * converted->w) + x] & fmt->Amask;
					alph = alph >> fmt->Ashift;
					alph = alph << fmt->Aloss;
					r = px[(y * converted->w) + x] & fmt->Rmask;
					r = r >> fmt->Rshift;
					r = r << fmt->Rloss;
					g = px[(y * converted->w) + x] & fmt->Gmask;
					g = g >> fmt->Gshift;
					g = g << fmt->Gloss;
					b = px[(y * converted->w) + x] & fmt->Bmask;
					b = b >> fmt->Bshift;
					b = b << fmt->Bloss;
					// Solid black
					if (r == 0 && g == 0 && b == 0)
					{
						...
						if ((x + 2) <= converted->w) {
							if (!check_solid_pixel(converted, x + 1, y) && check_solid_pixel(converted, x + 2, y))
							{
								alph /= 2;
								r /= 2;
								g /= 2;
								b /= 2;
								//Convert adjacent to same colour with modified alpha
								px[((y * converted->w) + x + 1)] = SDL_MapRGBA(converted->format, r, g, b, alph);
							}
						}
						// This pixel is black next to alpha, convert
						if (check_solid_pixel(converted, x, y) && check_alpha_pixel(converted, x - 1, y)) {
							px[((y * converted->w) + x)] = SDL_MapRGBA(converted->format, r, g, b, alph);
						}

					}
					//Alter the pixel itself
					px[((y * converted->w) + x)] = SDL_MapRGBA(converted->format, r, g, b, alph);
				}
			}
			SDL_UnlockSurface(converted);
			GFX_k[myslot].k = g_display->upload(converted);
		}
		else {
		GFX_k[myslot].k = g_display->upload(surf);
		}
