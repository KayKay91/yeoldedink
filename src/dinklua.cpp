...
static int lua_load_script(char *path, int script)
{
...
static int lua_proc_exists(int script, char *proc)
{
...
static int lua_run_script_proc(int script, char *proc)
{
  char *proc_name = strdup(proc);
  strtolower(proc_name);
...
void dinklua_initialize(struct script_engine *script_engine)
{
  ...
  luaVM = luaL_newstate();
  luaL_openlibs(luaVM);
  //Explicitly load safe libraries instead, comment out above
  //sol::state_view lua(luaVM);
  //lua.open_libraries(sol::lib::base, sol::lib::coroutine, sol::lib::debug, sol::lib::math, sol::lib::package, sol::lib::string, sol::lib::table);
  dinklua_bind_init();
  ...
  char *path = paths_pkgdatafile("init.lua");
  if (path == NULL)
  {
...
//For debug console, doesn't work fully yet
void dinklua_execute_one_liner(char* one_liner)
{
  // Get our existing state
  sol::state_view lua(luaVM);
  lua.script("player = dink.get_sprite(1)");
  auto result = lua.safe_script(one_liner, sol::script_pass_on_error);
  // Expel an error message
  if (!result.valid()) {
    sol::error err = result;
    log_error("🌒 Lua error: %s", err.what());
  }

}

//So you don't have to restart maybe. Not sure if it works properly
//it hoses the scripting environment!
void dinklua_reload_init()
{
char *path = paths_pkgdatafile("init.lua");
  if (path == NULL)
  {
...
//ye: clear volatile table upon restart
void dinklua_clear_globals()
{
  log_debug("Resetting volatile table");
  sol::state_view lua(luaVM);
  lua.script("for k,v in pairs(volatile) do volatile[k] = nil end");
}
