...
void App::print_version() {
	...
	printf("YeOldeDink is free software, and you are welcome to redistribute "
		"it\n");
...
void App::print_help(int argc, char* argv[]) {
	...
	printf(_("  -S, --software-rendering  Do use OpenGL\n"));
	printf(_("  -c, --config <yedink.ini>  Specify a config file\n"));
	printf("\n");
...
bool App::check_arg(int argc, char* argv[]) {
	...
	char short_options[] = "drc:g:hijsvw7tS";
	...
	//yeolde: message box
	if (!truecolor && !windowed && dinkgl && sound_on && joystick && dmoddir_opt == NULL && !debug_p) {
		//Dink started with no cmdline arguments
		...
		mbox.message = "Pick your YeOldeDink launch options";
		mbox.numbuttons = 3;
		mbox.title = "Choose a display mode to run the main game, or select a D-mod to launch or decompress.";
		mbox.window = NULL;
		//mbox.flags = SDL_MESSAGEBOX_WARNING;
		SDL_ShowMessageBox(&mbox, &butt);
		...
		if (butt == 1) {
			...
			int butty;
			//Display a picker using tinyfiledialogs
			dmoddir_opt = strdup(tinyfd_selectFolderDialog("Select your D-Mod location", NULL));
			//Display a second box with truecolour and window options
			const SDL_MessageBoxButtonData morebuttons[] = {
				{1, 0, "Windowed 24-bit"},
				{0, 1, "Full-screen 24-bit"},
				{0, 2, "Windowed 8-bit"},
				{0, 3, "Full-screen 8-bit"},
			};
			mbox.buttons = morebuttons;
			mbox.message = "If in doubt, select 24-bit.";
			mbox.numbuttons = 4;
			mbox.title = "Select your display mode for this D-mod";
			SDL_ShowMessageBox(&mbox, &butty);
			if (butty == 0) {
				windowed = true;
				truecolor = true;
			}
			...
			//show file picker. filter doesn't seem to work too great
			//const char* filters[] = {"*.dmod"};
			char* mydmod = strdup(tinyfd_openFileDialog("Select a .dmod file to decompress", NULL, 0, NULL, "dmod bz2 file", 0));
	...
	//#ifndef DINKEDIT
	//ye: Load config file after paths initted
	//TODO: move this elsewhere
	CSimpleIniA yedink;
	yedink.SetUnicode();
	if (conf_opt != NULL)
	yedink.LoadFile(conf_opt);
	else {
	yedink.LoadFile(paths_pkgdatafile("yedink.ini"));
	}
	strcpy(debugfon, yedink.GetValue("fonts", "debugttf", "Roboto-Medium.ttf"));
	strcpy(dispfon, yedink.GetValue("fonts", "display", "LiberationSans-Regular.ttf"));
	window_w = atoi(yedink.GetValue("display", "window_w", "640"));
	window_h = atoi(yedink.GetValue("display", "window_h", "480"));
	strcpy(debug_editor, yedink.GetValue("display", "textedit", "code"));
	debug_fontsize = atoi(yedink.GetValue("fonts", "debug_pt_size", "14"));
	audio_samplerate = atoi(yedink.GetValue("audio", "samplerate", "44100"));
	#ifndef DINKEDIT
	midiplayer = atoi(yedink.GetValue("audio", "midiplayer", "0"));
	#endif
	adlemu = atoi(yedink.GetValue("audio", "adlemu", "0"));
	opnemu = atoi(yedink.GetValue("audio", "adlemu", "0"));
	hw_channels = atoi(yedink.GetValue("audio", "channels", "2"));
	strcpy(soundfont, yedink.GetValue("audio", "soundfont", "TimGM6MB.sf2"));
	//#endif
	free(conf_opt);
...
avoid stucking the user in 640x480 when crashing) */
	...
	// TODO: move to a separate D-Mod object
	{
		...
		int nb_idata = (opt_version >= 108) ? 2000 : 600;
		...
		load_hard();
		log_info("🗺️ World data....");
		g_dmod.map.load();
		log_info("✅ done!");
		log_info("📇 Loaded Dink.dat");
		spdlog::debug("Loaded in {:.3} seconds", sw);

		//imgui.ini
		ImGui::LoadIniSettingsFromDisk(paths_pkgdatafile("imgui.ini"));
	}
...
void App::loop() {
	/* Main loop */
#ifdef __EMSCRIPTEN__
	emscripten_set_main_loop_arg(myemscripten_loop, this, 65, 1);
#else
	while (run) {
		one_iter();
	}
