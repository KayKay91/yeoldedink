...
int dversion = 108;
//For get client fork, to determine which engine is running. Please increment if you fork.
int dfork = 3;
...
Uint64 cycle_clock = 0;
int timer_fade = 1000;
/* What script to resume after fade: */
int cycle_script = 0;

//ye: custom experience points
int customexp = 0;
...
int please_wait_toggle_frame = 7;
...
Uint64 lastTickCount;
Uint64 pauseTickCount;
int fps_final = 0;
float base_timing = 0;
...
void game_restart() {
	...
	//ye: dinklua clear volatile table
	if (dinklua_enabled)
	...
	mainscript = scripting_load_script("main", 0, 1);
 ...
 */
Uint64 game_GetTicks() {
	static Uint64 last_sdl_ticks = 0;
	static Uint64 high_ticks = 0;
	static Uint64 pause_ticks = 0;

	if (pauseTickCount > 0) {
		pause_ticks = SDL_GetTicks64() - pauseTickCount;
	}

	Uint64 cur_sdl_ticks = SDL_GetTicks64() - pause_ticks;
	pauseTickCount = 0;
	...
	//Turbo speed, 6x
	if (high_speed == 2) {
		high_ticks += 5 * (cur_sdl_ticks - last_sdl_ticks);
	}
	//Slow speed
	if (high_speed == -1) {
		high_ticks -= 0.5 * (cur_sdl_ticks - last_sdl_ticks);
	}

	if (high_speed == -2) {
		high_ticks -= 0.9 * (cur_sdl_ticks - last_sdl_ticks);
	}
...
void game_set_turbo_speed() {
	if (high_speed != 2) {
		#ifndef __EMSCRIPTEN__
		SDL_setFramerate(&framerate_manager, 6 * FPS);
		#endif
		if (!dinklua_enabled)
			set_music_tempo(6.0);
...
//Yeolde: Added this
void game_set_slow_speed() {
	if (high_speed != -1) {
	#ifndef __EMSCRIPTEN__
	SDL_setFramerate(&framerate_manager, 0.5 * FPS);
	#endif
	if (!dinklua_enabled)
		set_music_tempo(0.5);
	high_speed = -1;
...
void game_set_extremely_slow_speed() {
	if (high_speed != -2) {
	#ifndef __EMSCRIPTEN__
	SDL_setFramerate(&framerate_manager, 0.1 * FPS);
	#endif
	if (!dinklua_enabled)
		set_music_tempo(0.2);
	high_speed = -2;
...
void add_exp_force(int num, int source_sprite) {
	if (num > 0) {
		...
		*pexp += num;
		...
		spr[crap2].x -=
				k[seq[spr[source_sprite].pseq].frame[spr[source_sprite].pframe]]
						.xoffset;
		spr[crap2].y -=
				k[seq[spr[source_sprite].pseq].frame[spr[source_sprite].pframe]]
						.box.bottom /
...
void fix_dead_sprites() {
	...
	for (i = 1; i < 100; i++) {
		...
		if (type == 6) {
			if ((thisTickCount >
				(play.spmap[*pplayer_map].last_time + 300000)) ||
...
int game_load_screen(int mapdat_num) {
	if (g_dmod.map.ts_loc_mem[mapdat_num] != NULL && mapdat_num < 768) {
		//log_error("mapdat num is %i, size %i", mapdat_num, sizeof(g_dmod.map.ts_loc_mem[mapdat_num]));
		memcpy(&cur_ed_screen, g_dmod.map.ts_loc_mem[mapdat_num], sizeof(struct editor_screen));
	} else if (load_screen_to(g_dmod.map.map_dat.c_str(), mapdat_num,
							&cur_ed_screen) < 0) {
		return -1;
	}
		...
		if (cur_ed_screen.sprite[j].active)
			if (play.spmap[*pplayer_map].type[j] != 0) {
				...
				cur_ed_screen.sprite[j].frame =
						play.spmap[*pplayer_map].frame[j];
...
void game_place_sprites() {
	...
	for (; r1 < MAX_SPRITES_EDITOR && rank[r1] > 0; r1++) {
		...
		if (cur_ed_screen.sprite[j].active == 1 &&
			...
			cur_ed_screen.sprite[j].vision == *pvision || debug_vision)) {
			...
			if (cur_ed_screen.sprite[j].type == 0 ||
				...
				if (cur_ed_screen.sprite[j].type == 0 || (cur_ed_screen.sprite[j].type == 2 && debug_invspri))
		...
		add_hardness(sprite, 1); else */
			...
			add_hardness(sprite, 1);
				...
				if (strlen(cur_ed_screen.sprite[j].script) > 1) {
					spr[sprite].script =
							scripting_load_script(cur_ed_screen.sprite[j].script, sprite, 1);
				}
			}
...
void game_place_sprites_background() {
	...
	for (; r1 < MAX_SPRITES_EDITOR && rank[r1] > 0; r1++) {
		...
		if (cur_ed_screen.sprite[j].active == 1 &&
			(cur_ed_screen.sprite[j].vision == 0 ||
			cur_ed_screen.sprite[j].vision == *pvision)) {
			if (cur_ed_screen.sprite[j].type == 0 || (cur_ed_screen.sprite[j].type == 2 && debug_invspri)) {
				//we have instructions to make a sprite
				check_seq_status(cur_ed_screen.sprite[j].seq);
	...
	for (; k < MAX_SCRIPTS; k++) {
		if (sinfo[k] != NULL &&
			...
			if (scripting_proc_exists(k, "main")) {
				log_debug("🎯 Screendraw: running main of script %s..",
						sinfo[k]->name);
...
void draw_screen_game(void) {
	*pvision = 0;
	...
	if (cur_ed_screen.ts_script_id > 0)
		...
		script_id = scripting_load_script(cur_ed_screen.script, 0, 1);

	//For running screen script
	if (script_id > 0) {
		log_info("📺 Screen script found, attempting to run main()");
		bool main_exist = scripting_proc_exists(script_id, "MAIN");
		screen_main_is_running = /*true*/ 1;
		//ye: hack for bftg
		if (main_exist)
			scripting_run_proc(script_id, "MAIN");
		else
			scripting_resume_script(script_id);
	...
	if (debug_mode && dbg.debug_meminfo) {
...
static void draw_wait() {
	if (screen_main_is_running) {
		log_info("📺 Screen main() is running, showing waiting indicator");
		if (seq[423].frame[please_wait_toggle_frame] != 0 && dbg.showplswait) {
...
void set_mode(int new_mode) {
	...
	//} else {
		...
		//Yeolde: I hate this shit, turning it off
		SDL_SetWindowGrab(g_display->window, SDL_FALSE);
	//}
}
 ...
 */
void apply_mode0() {
	log_info("🐁 Applying mode zero to prepare mode 1 and running start script.");
...
errors properly instead of using this dirty trick. */
	...
	if (dversion >= 108)
		dversion_string = "YeOldeDink in 1.08 mode";
	else
		dversion_string = "YeOldeDink in 1.07 mode";
