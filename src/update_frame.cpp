...
void check_joystick() {
	...
	//Start+Select ragequit
	#ifndef __EMSCRIPTEN__
	if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_BACK) && SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_START) && dbg.jquit) {
		SDL_Event ev;
		ev.type = SDL_QUIT;
		SDL_PushEvent(&ev);
	}
	...
	if (joystick && !debug_mode) {
		SDL_GameControllerUpdate();
		Sint16 x_pos = 0, y_pos = 0, action_pos = 0, action_pos2 = 0;
		/* SDL counts buttons from 0, not from 1 */
		//Yeolde: muh manual keybinds
		//for (int i = 0; i < 20; i++) {
			if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_A))
				sjoy.joybit[ACTION_ATTACK] = 1;
			if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_B))
				sjoy.joybit[ACTION_TALK] = 1;
			if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_Y))
				sjoy.joybit[ACTION_INVENTORY] = 1;
			if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_X))
				sjoy.joybit[ACTION_MAGIC] = 1;
			if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_RIGHTSHOULDER))
				sjoy.joybit[ACTION_SPEED] = 1;
			if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_START))
				sjoy.joybit[ACTION_MENU] = 1;
			if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_BACK))
				sjoy.joybit[ACTION_MAP] = 1;
			//Assignable
			if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_LEFTSHOULDER))
				sjoy.joybit[ACTION_BUTTON7] = 1;
		//}
		x_pos = SDL_GameControllerGetAxis(ginfo, SDL_CONTROLLER_AXIS_LEFTX);
		y_pos = SDL_GameControllerGetAxis(ginfo, SDL_CONTROLLER_AXIS_LEFTY);
		/* Using thresold=10% (original game) is just enough to get rid
	of the noise. Let's use 30% instead, otherwise Dink will go
	diags too easily. */
		{
			Sint16 threshold = 32767 * dbg.joythresh / 100;
			if (x_pos < -threshold)
				sjoy.left = 1;
			if (x_pos > +threshold)
				sjoy.right = 1;
		...
		//Yeolde: d-pad too
		if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_DPAD_LEFT))
			sjoy.left = 1;
		if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_DPAD_RIGHT))
			sjoy.right = 1;
		if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_DPAD_UP))
			sjoy.up = 1;
		if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_DPAD_DOWN))
			sjoy.down = 1;

		//Yeolde: let's add the left and right triggers (L2 and R2 on a sixaxis etc)
		if (SDL_GameControllerGetAxis(ginfo, SDL_CONTROLLER_AXIS_TRIGGERLEFT) > 2000)
			sjoy.joybit[ACTION_MAGIC] = 1;
		if (SDL_GameControllerGetAxis(ginfo, SDL_CONTROLLER_AXIS_TRIGGERRIGHT) > 2000)
			sjoy.joybit[ACTION_ATTACK] = 1;

		//Debug on/off with clickys
		//TODO: make a timer
		if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_LEFTSTICK))
			log_debug_on(dbg.loglevel);
	}


		if (SDL_GameControllerGetButton(ginfo, SDL_CONTROLLER_BUTTON_RIGHTSTICK) && joystick && debug_mode)
			log_debug_off();



	if (input_getscancodestate(SDL_SCANCODE_LCTRL) ||
		input_getscancodestate(SDL_SCANCODE_RCTRL))
		sjoy.joybit[ACTION_ATTACK] = 1;
	if (input_getscancodestate(SDL_SCANCODE_SPACE))
		sjoy.joybit[ACTION_TALK] = 1;
	if (input_getscancodestate(SDL_SCANCODE_LSHIFT) ||
		input_getscancodestate(SDL_SCANCODE_RSHIFT))
		sjoy.joybit[ACTION_MAGIC] = 1;
	...
	if (input_getscancodestate(SDL_SCANCODE_RIGHT) || sjoy.joybit[ACTION_RIGHT])
		sjoy.right = 1;
	if (input_getscancodestate(SDL_SCANCODE_LEFT) || sjoy.joybit[ACTION_LEFT])
		sjoy.left = 1;
	if (input_getscancodestate(SDL_SCANCODE_DOWN) || sjoy.joybit[ACTION_DOWN])
		sjoy.down = 1;
	if (input_getscancodestate(SDL_SCANCODE_UP) || sjoy.joybit[ACTION_UP])
		sjoy.up = 1;
	...
	/* High speed */
	//Yeolde: only triggers in normal mode now.
	if (!debug_mode) {
	if (input_getscancodestate(SDL_SCANCODE_TAB) == 1 || sjoy.joybit[ACTION_SPEED]) {
		game_set_high_speed();
	...
	//Yeolde: added this for mouse mode for the cases of the lost cursor
	if (input_getscancodestate(SDL_SCANCODE_TAB) && (mode == 1) && dbg.mouseloc) {
		spr[1].x = 640 / 2;
		spr[1].y = 480 / 2;
	}
...
trigger_start:
	...
	for (int j = 0; j <= max_s; j++) {
		...
		if (live_sprite_is_expired(h, thisTickCount)) {
			...
			if (spr[h].say_stop_callback != 0)
				scripting_resume_script(spr[h].say_stop_callback);
		}
	...
	/* Screen transition */
	if (get_frame && dbg.screentrans) {
		get_frame = false;
		transition_in_progress = 1;
		//ye: autosave
		//TODO: put this somewhere else
		if (dbg.autosave)
			save_game(dbg.autosaveslot);
	...
	//Switch off normal text if alttext view enabled
	//if (!dbg.alttext || debug_mode) {
	for (int j = 0; j <= max_s; j++) {
		int sprite = rank[j];
		if (sprite > 0 && spr[sprite].active && spr[sprite].brain == 8)
			if ((debug_alttext && spr[sprite].damage != -1) || (!debug_alttext || debug_mode))
	...
	scripting_process_callbacks(thisTickCount);
