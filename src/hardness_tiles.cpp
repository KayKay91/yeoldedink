...
void load_hard(void) {
	...
	ProgressBar bar2 {
		...
		option::MaxProgress{HARDNESS_NB_TILES},
		option::PrefixText{"Hard.dat squares: "}
	};

	ProgressBar bar3 {
		option::BarWidth{80},
		option::ForegroundColor{Color::green},
		option::MaxProgress{GFX_TILES_NB_SQUARES},
		option::PrefixText{"Hard.dat indices: "}
	};

	DynamicProgress<ProgressBar> bars(bar2, bar3);
	...
	int i = 0;
	for (i = 0; i < HARDNESS_NB_TILES; i++) {
		//memset(&hmap.htile[i], mmap[i * 2608], sizeof(ts_block));
		for (int x = 0; x < 50 + 1; x++)
			for (int y = 0; y < 50 + 1; y++)
		...
		if (debug_mode && dbg.progressbars)
			bars[0].tick();
	}
	if (debug_mode)
	...
	for (i = 0; i < GFX_TILES_NB_SQUARES; i++) {
		...
		if (hmap.btile_default[i] < 0 || hmap.btile_default[i] > 799) {
			bars[1].set_option(option::ForegroundColor{Color::red});
			bars[1].set_option(option::PostfixText{"Bad index detected"});
		}
		}
	}
	fseek(f, (8000 - GFX_TILES_NB_SQUARES) * 4, SEEK_CUR); // reproduce unused data
