...
SDL_Color font_extra_colors[16];
...
provided). */
//yeolde: for the user's sake we'll just pick a different typeface now
...
static TTF_Font* load_default_font() {
	...
	/* Try from resources */
	if (rwops == NULL) {
		rwops = find_resource_as_rwops(dispfon);
		if (rwops != NULL)
			log_info("🔤 Loaded display font from %s",
					paths_getpkgdatadir());
	}
#ifdef HAVE_FONTCONFIG
	if (rwops == NULL) {
		char* path = get_fontconfig_path(dispfon);
		rwops = SDL_RWFromFile(path, "rb");
		if (rwops != NULL) {
			log_info("🔤 Loaded %s from "
					"fontconfig at %s",
					dispfon, path);
		free(path);
		}
		else {
			char* path = get_fontconfig_path("OpenSans-Regular:style=Regular");
			rwops = SDL_RWFromFile(path, "rb");
			if (rwops != NULL) {
			log_info("🔤 Loaded OpenSans from Fontconfig at %s", path);
			free(path);
			} else {
				//Show a box allowing for a chosen typeface
				const char* filters[] = {"*.ttf", "*.fon"};
				rwops = SDL_RWFromFile(tinyfd_openFileDialog("Display font not found. Please choose a TTF typeface", NULL, 1, filters, NULL, 0), "rb");
			}
		}
	}
#endif
...
#endif
//yeolde: let's enable fontconfig on woe too. doesn't seem to work properly yet
#ifdef HAVE_FONTCONFIG
		if (path == NULL)
	...
	//yeolde: let's also search in dmodfile
	if (path == NULL) {
		path = paths_dmodfile(filename);
	}
	...
	while (!this_is_last_line) {
		...
		if (!calc_only) {
			...
			if (img != NULL) {
				...
				SDL_Rect src;
				src.x = src.y = 0;
				src.w = std::min(w, img->w); // truncate text if outside the box
				src.h = img->h;
...
IOGfxSurface* print_text_flatten_cmds(std::vector<TextCommand>* cmds) {
	...
	for (std::vector<TextCommand>::iterator it = cmds->begin();
		it != cmds->end(); ++it) {
		min_x = std::min(min_x, it->dst.x);
		min_y = std::min(min_y, it->dst.y);
		max_x = std::max(max_x, it->dst.x + it->src.w);
		max_y = std::max(max_y, it->dst.y + it->src.h);
	}
	SDL_Surface* flat =
...
void gfx_fonts_init_extra_colors()
{
	//ye: extra ones from rtdink listed in font_big.txt
	//this one is labelled 'default' and is some sort of grey
	font_extra_colors[1] = {230, 230, 230};

	//dark green
	font_extra_colors[2] = {0, 160, 0};

	//dark cyan
	font_extra_colors[3] = {0, 160, 160};

	//profondo rosso
	font_extra_colors[4] = {198, 5, 5};

	//deep purple
	font_extra_colors[5] = {160, 0, 114};

	//brown
	font_extra_colors[6] = {210, 195, 172};

	//sasha grey
	font_extra_colors[7] = {160, 160, 160};

	//darker brown
	font_extra_colors[8] = {132, 108, 70};

	//'dark brown' which is actually blue
	font_extra_colors[9] = {0, 0, 255};

	//bright green
	font_extra_colors[10] = {0, 255, 0};

	//bright cyan
	font_extra_colors[11] = {0, 255, 255};

	//bright red
	font_extra_colors[12] = {232, 0, 0};

	//bright purple
	font_extra_colors[13] = {232, 0, 166};

	//bright yellow
	font_extra_colors[14] = {252, 255, 0};

	//white
	font_extra_colors[15] = {255, 255, 255};
}
