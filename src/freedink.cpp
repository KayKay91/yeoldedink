...
//ye: for json later on
int map_width = 32;
int number_of_screens = 768;

//timers
int timer_notouch = 400;
int timer_showbmp = 2000;
Uint64 timer_fadetime = 400.0;
...
auto palcycle = tweeny::from(256).to(0).via(easing::quarticOut).during(400);
/**
...
int check_if_move_is_legal(int u) {
	if ((dversion >=
		108) /* move_nohard is active for all movements, not just active moves */
	...
	if (spr[u].moveman > 0) {
		...
		for (i = 1; i <= spr[u].moveman; i++) {
			...
			if ((hardness == 2 && spr[u].flying) || debug_noclip) {
				spr[u].moveman = 0;
				if (dversion >= 108)
					return 0;
				else
			...
			if (hardness > 0) {
				...
				if (push_active) {
					if (u == 1 && hardness != 2 && play.push_active == false) {
						if ((spr[u].dir == 2) | (spr[u].dir == 4) |
							(spr[u].dir == 6) | (spr[u].dir == 8)) {
...
int did_player_cross_screen() {
	if (walk_off_screen == 1 || debug_walkoff)
		return 0;

	int ret = 0;
	// DO MATH TO SEE IF THEY HAVE CROSSED THE SCREEN, IF SO LOAD NEW ONE
	if (spr[1].x < playl) {
		if ((*pplayer_map - 1) >= 1 && g_dmod.map.loc[*pplayer_map - 1] > 0 &&
			screenlock == 0) {
	...
	if (spr[1].x > 619) {
		if ((*pplayer_map + 1) <= number_of_screens && g_dmod.map.loc[*pplayer_map + 1] > 0 && screenlock == 0) {
	...
	if (spr[1].y < 0) {
		if ((*pplayer_map - map_width) >= 1 && g_dmod.map.loc[*pplayer_map - map_width] > 0 &&
			screenlock == 0) {
			//move one map up
			update_screen_time();
			grab_trick(8);
			*pplayer_map -= map_width;
			game_load_screen(g_dmod.map.loc[*pplayer_map]);
			if (g_dmod.map.indoor[*pplayer_map] == 0)
				play.last_map = *pplayer_map;
	...
	if (spr[1].y > 399) {
		if ((*pplayer_map + map_width) <= number_of_screens && g_dmod.map.loc[*pplayer_map + map_width] > 0 && screenlock == 0) {
			//move one map down
			update_screen_time();
			grab_trick(2);
			*pplayer_map += map_width;
			...
			if (i != h)
				if (spr[i].brain != 8) {
					...
					if (spr[h].dir == 2) {
						...
						if (debug_talkboxexp && spr[i].brain == 16)
							box.top -= amounty;
					}
					...
					if (debug_mode && debug_talksquares) {
					SDL_Rect r = {box.left, box.top, box.right - box.left,
								box.bottom - box.top};
					IOGFX_backbuffer->fillRect(&r, 0, 255, 10);
				}
...
int special_block(int block) {
	if (cur_ed_screen.sprite[block].is_warp == 1) {
		...
		if (cur_ed_screen.sprite[block].sound == 0)
			...
			SoundPlayEffect(cur_ed_screen.sprite[block].sound, sfx_spritehz, 0, 0, 0);
...
void CyclePalette() {
	if (!truecolor) {
		...
		if (truecolor_fade_lasttick == 0) {
			...
			if (dbg.altfade) {
			palcycle.forward();
			g_display->brightness = palcycle.step(delta);
			}
			else {
				g_display->brightness -= delta * 256 / timer_fadetime;
			}
		}
	...
	if (process_downcycle) {
		if (thisTickCount > cycle_clock) {
			palcycle.during(timer_fadetime);
...
void up_cycle(void) {
	...
	if (!truecolor) {
		...
		if (truecolor_fade_lasttick == 0) {
			...
			if (dbg.altfade) {
			palcycle.backward().via(easing::circularIn);
			g_display->brightness = palcycle.step(delta);
			}
			else {
				g_display->brightness += delta * 256 / timer_fadetime;
		...
		if (spr[i].active)
			if (i != h)
				if (!((spr[i].nohit == 1) && (spr[i].script == 0))) {
					...
					if (debug_mode && debug_hitboxsquares) {
						SDL_Rect r = {box.left, box.top, box.right - box.left,
									box.bottom - box.top};
						IOGFX_backbuffer->fillRect(&r, 222, 173, 155);
					}
					...
					if (inside_box(spr[h].x, spr[h].y, box)) {
						...
						//select in editor/viewer
						if (dbg.punchedit) {
								spriedit = i;
							}

						//player sprite hit
						if (debug_pausehit && i == 1) {
							debug_paused = true;
							pause_everything();
						}
						...
						if (spr[i].nohit == 1) {
							...
							if (spr[h].attack_hit_sound == 0) {
								sfx_play_hit_thwack();
								log_debug("💥 Playing default attack sound effect after %d hitting %d", h, i);
							} else {
								SoundPlayEffect(spr[h].attack_hit_sound,
												spr[h].attack_hit_sound_speed,
												0, 0, 0);
							}
							...
							if (spr[h].strength == 0) {
								...
								if ((spr[i].hitpoints > 0) || (i == 1)) {
									...
									spr[i].last_hit = h;
									if (hurt_thing(i, (spr[h].strength / 2) + ((Random::get(1, spr[h].strength) / 2) + 1), 0) > 0)
		...
		if (spr[i].active)
			if (i != h)
				if ((spr[i].touch_damage != 0)) {
					...
					box.top -= 2;
					box.bottom += 2;
					if (debug_mode && dbg.debug_pinksquares) {
						SDL_Rect r = {box.left, box.top, box.right - box.left,
									box.bottom - box.top};
					...
					if (inside_box(spr[h].x, spr[h].y, box)) {
						...
						if ((spr[i].touch_damage == -1) &&
							...
							if (spr[i].touch_damage == -1) {
								log_error("💔 Sprites touch damage is set to -1 "
										"but there is no script set!");
							} else {
								//lets hurt the guy

								spr[h].notouch = /*true*/ 1;
								spr[h].notouch_timer = thisTickCount + timer_notouch;
								spr[h].last_hit = i;
								if (spr[i].script != 0)
									scripting_run_proc(spr[i].script, "TOUCH");
...
void process_show_bmp(void) {
	//Crappy fix for bmps not showing while faded down like in Mystery Island
	//Does show bmp cancel a fade in 1.08?
	if (truecolor)
		g_display->brightness = 256;
	...
	if (showb.showdot) {
		...
		showb.picframe++;
		if (showb.picframe > seq[mseq].len)
			showb.picframe = 1;
	...
	if ((sjoy.button[ACTION_ATTACK]) || (sjoy.button[ACTION_TALK]) ||
		(sjoy.button[ACTION_MAGIC]) || (sjoy.button[ACTION_INVENTORY]) ||
		(sjoy.button[ACTION_MENU]) || (sjoy.button[ACTION_MAP])) {
		showb.active = /*false*/ 0;
		if (showb.script != 0)
			scripting_resume_script(showb.script);
		//ye: this doesn't appear to be used...
		showb.stime = thisTickCount + timer_showbmp;
		but_timer = thisTickCount + 200;
		//ye: why the hell does it make a text sprite?
