...
void game_choice_start(int script, int nb_choices) {
	...
	//if (ret == -1)
	//	log_error("🐀 Relative mouse positioning not supported on this platform.");
...
void game_choice_logic() {
	...
	if ((!debug_mode || dbg.clickmouse) && play.mouse > 20) {
		game_choice.cur++;
		play.mouse = 0;
	}
	//don't scroll the menu in debug mode
	if ((!debug_mode || dbg.clickmouse) && play.mouse < -20) {
		game_choice.cur--;
		play.mouse = 0;
	}
...
begin:
	...
	if (talk_hold != game_choice.cur && game_choice.cur >= game_choice.cur_view && game_choice.cur <= game_choice.cur_view_end)
		sfx_play_choice_scroll();
	...
	//tabulate distance needed by text, LORDII experience helped here
	{
		...
		for (; i < game_choice.last; i++) {
			...
			y_hold = print_text_wrap(game_choice.line[i], &rcRect, 1, 1,
									FONT_DIALOG);
			sy_hold += y_hold;
	...
	if (game_choice.cur_view == 1 &&
		game_choice.cur_view_end == game_choice.last) {
...
death:
	...
	if (game_choice.timer < thisTickCount) {
		game_choice.curf++;
		game_choice.timer = thisTickCount + timer_arrow;
	}
	if (game_choice.curf == 0)
		game_choice.curf = 1;
		//yeolde: changed this to use all the seq
	if (game_choice.curf > seq[457].len)
		game_choice.curf = 1;

	// Return to script on choice
	if ((!debug_mode || dbg.clickmouse) && (sjoy.button[ACTION_ATTACK] || mouse1)) {
		mouse1 = /*false*/ 0;
		game_choice_stop();
		*presult = game_choice.line_return[game_choice.cur];
		sfx_play_choice_select();

		if (game_choice.script != 0) {
			//we need to continue a script
			scripting_resume_script(game_choice.script);
		}
	}
}
