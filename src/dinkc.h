...
//For getting script contents in imgui
extern char* rinfo_code[MAX_SCRIPTS];
...
//New bindings system
extern std::unordered_map<std::string, binding> dinkc_bindings;
...
//extern int dinkc_load_script(char *filename, int sprite);
...
extern void dinkc_make_int(char* name, int value, int scope, int script);
extern void var_equals(char name[20], char newname[20], char math, int script,
					char rest[200]);
...
extern struct binding* dinkc_bindings_lookup(char* funcname);
