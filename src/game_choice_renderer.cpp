...
SDL_Color choice_text = {255, 255, 2};
SDL_Color choice_text_hover = {255, 255, 255};
...
void game_choice_renderer_render() {
	...
	}
	{
		...
		if (IOGFX_backbuffer->blit(GFX_k[seq[30].frame[4]].k, NULL, &dst) < 0)
			log_error("🖍️ Could not draw sprite %d: %s", seq[30].frame[4],
					SDL_GetError());
	}
	...
	for (int i = game_choice.cur_view; i <= game_choice.cur_view_end; i++) {
		...
		FONTS_SetTextColor(fg.r, fg.g, fg.b);
		rect_offset(&rcRect, 1, 1);
		y_last = print_text_wrap(game_choice.line[i], &rcRect, 1, 0,
								FONT_DIALOG);
	...
	// Draw arrows
	{
		...
		dst.x = curxl;
		dst.y = curyl;
		IOGFX_backbuffer->blit(GFX_k[seq[456].frame[game_choice.curf]].k, NULL,
							&dst);
		dst.x = curxr;
		dst.y = curyr;
		IOGFX_backbuffer->blit(GFX_k[seq[457].frame[game_choice.curf]].k, NULL,
							&dst);
	}
}

void scripting_set_choice_colour(uint8_t r, uint8_t g, uint8_t b) {
	choice_text.r = r;
	choice_text.g = g;
	choice_text.b = b;
}

void scripting_set_choice_hover_colour(uint8_t r, uint8_t g, uint8_t b) {
	choice_text_hover = {r, g, b};
}
