...
void live_sprite_set_kill_start(int h, Uint64 thisTickCount) {
...
bool live_sprite_is_expired(int h, Uint64 thisTickCount) {
 ...
 */
void live_sprite_animate(int h, Uint64 thisTickCount, bool* p_isHitting) {
	if (spr[h].reverse) {
			...
			if (spr[h].frame < 1) {
				...
				if (thisTickCount > spr[h].delay) {
					...
					if (spr[h].seq > 0 && seq[spr[h].seq].special[spr[h].frame] == 1) {
						*p_isHitting = true;
						log_debug("🧨 Special frame in sprite %d is set to hit", h);
					}
				}
			}
		}
