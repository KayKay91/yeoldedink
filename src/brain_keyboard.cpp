...
int timer_bow = 100;
int timer_bow_pull = 10;
int timer_bow_max = 500;
int timer_push = 600;
		...
		if (spr[i].active)
			if (i != h)
				if ((spr[i].script != 0)) {
					...
					if (debug_mode && debug_pushsquares) {
					SDL_Rect r = {box.left, box.top, box.right - box.left,
								box.bottom - box.top};
					IOGFX_backbuffer->fillRect(&r, 240, 252, 10);
				}
				//ye: autopause upon push setting
				if (debug_pausepush) {
					debug_paused = true;
					pause_everything();
				}
...
void process_bow(int h) {
	...
	if (sjoy.right)
		if (sjoy.up) {
			spr[h].dir = 9;
			bow.wait = thisTickCount + timer_bow;
		}
	if (sjoy.left)
		if (sjoy.up) {
			spr[h].dir = 7;
	...
	if (bow.pull_wait < thisTickCount) {
		bow.pull_wait = thisTickCount + timer_bow_pull;
		if (bow.hitme)
			bow.time += 7;

		//	bowsound->SetFrequency(22050+(bow.time*10));

		if (bow.time > timer_bow_max)
			bow.time = timer_bow_max;
...
void human_brain(int h) {
	...
	if (spr[h].damage > 0) {
		...
		//yeolde: added this for infinite health cheat
		if (!debug_infhp)
			*plife -= spr[h].damage;
		...
		/*
		int hurt = Random::get<int>(1,2);

		if (hurt == 1) {
			SoundPlayEffect(15, 25050, 2000, 0, 0);
			if (dbg.jrumble)
				SDL_GameControllerRumble(ginfo, 0x0000, 0xFFFF, 62);
		}
		if (hurt == 2) {
			SoundPlayEffect(16, 25050, 2000, 0, 0);
			if (dbg.jrumble)
				SDL_GameControllerRumble(ginfo, 0xFFFF, 0x0000, 62);
		}
		*/
		//draw blood
	}
	...
	if (play.push_active && thisTickCount > play.push_timer + timer_push) {
	...
	if (sjoy.button[ACTION_TALK] == 1) {
		if (!run_through_tag_list_talk(h)) {
			...
			if (did_dnotalk == 0) {
				...
				int randy = Random::get<int>(1,6);
				//ye: german text strings from the 1.07 source
				if (dbg.germantext) {
				...
				if (randy == 1)
					say_text(_("`$I don't see anything here."), h, 0);
				if (randy == 2)
					say_text(_("`$Huh?"), h, 0);
				if (randy == 3)
					say_text(
							_("`$I'm fairly sure I can't talk to or use that."),
							h, 0);
				if (randy == 4)
					say_text(_("`$What?"), h, 0);
				if (randy == 5)
					say_text(_("`$I'm bored."), h, 0);
				if (randy == 6)
					say_text(_("`$Not much happening here."), h, 0);
				}
			}
		}
	}
	...
	if (thisTickCount > but_timer) {
		...
		for (scancode = 0; scancode < SDL_NUM_SCANCODES; scancode++) {
			if (scancode == SDL_SCANCODE_SPACE || scancode == SDL_SCANCODE_6 ||
				scancode == SDL_SCANCODE_7 || scancode == SDL_SCANCODE_LEFT ||
				scancode == SDL_SCANCODE_UP || scancode == SDL_SCANCODE_RIGHT ||
			...
			if (input_getscancodestate((SDL_Scancode)scancode)) {
				/* Get the same keycodes as the original Dink engines
		...
		lowercase ascii */
				...
				int script = scripting_load_script(scriptname, 1, 0);
				if (scripting_proc_exists(script, "MAIN")) {
...

	{
		...
		//ye: Hack this up for a custom keybind. Nobody's used button7.c ever
		for (int i = 0; i < nb_actions; i++) {
			...
			if (sjoy.button[actions[i]] == 1) {
				...
				if (actions[i] == ACTION_BUTTON7)
					sprintf(script_filename, "key-%d", dbg.fakekey);
				else
					sprintf(script_filename, "button%d", actions_script[i]);

				int mycrap = scripting_load_script(script_filename, 1, 0);
	...
	if (sjoy.button[ACTION_MAGIC] == 1) {
		if (magic_script == 0) {
			...
			if (dbg.germantext) {
				...
				if (randy == 3) say_text("`$Nichts ist passiert.",h,0);
				if (randy == 4) say_text("`$Hokus pokus!",h,0);
				goto b1end;
			}
			...
			kill_text_owned_by(h);
			if (randy == 1)
				say_text(_("`$I don't know any magic."), h, 0);
			if (randy == 2)
				say_text(_("`$I'm no wizard!"), h, 0);
			if (randy == 3)
				say_text(_("`$I need to learn magic before trying this."), h,
						0);
			if (randy == 4)
				say_text(_("`$I'm gesturing wildly to no avail!"), h, 0);
			if (randy == 5)
				say_text(_("`$Nothing happened."), h, 0);
			if (randy == 6)
				say_text(_("`$Hocus pocus!"), h, 0);
			goto b1end;
		}
	...
	shootm:
		if (*pmagic_level >= *pmagic_cost || debug_magcharge) {
	...
	if (spr[h].skip > 0 && spr[h].skip <= spr[h].skiptimer) {
		spr[h].skiptimer = 0;
		goto b1end;
	}
...
b1end:
	...
	if (spr[h].dir == 2 || spr[h].dir == 4 || spr[h].dir == 6 ||
		spr[h].dir == 8)
		goto smoothend;
				...
				if (hardm == 0) {
					...
					if (hardm == 0) {
						...
						if (hardm == 0) {
							...
							int hardm =
									get_hard_play(spr[h].x - 1, spr[h].y - 1);
							if (hardm == 0) {
								spr[h].x -= 1;
								spr[h].y -= 1;
							}
