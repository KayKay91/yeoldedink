...
void text_brain(int h) {
	if (!lsm_isValidSprite(h) ||
		(spr[h].text_owner != 0 && spr[h].text_owner != 1000 &&
		!lsm_isValidSprite(spr[h].text_owner))) {
		lsm_remove_sprite(h);
		return;
	}
	...
	if ((spr[h].damage == -1) && (spr[h].text_owner != 1000)) {
		...
		if (spr[h].y < 1)
			spr[h].y = 1;
		//ye: does this fix the right side?
		//only if you listen to robj and account for the width of the text box!
		if (spr[h].x > playx - 150)
			spr[h].x = playx - 150;
