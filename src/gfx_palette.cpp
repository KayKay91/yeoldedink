...
//ye: lua
void gfx_palette_set_index(int idx, int r, int g, int b) {
	//copy the existing
	SDL_Color mypal[256];
	gfx_palette_get_phys(mypal);
...
std::tuple<int, int, int> gfx_palette_get_index(int idx) {
	SDL_Color mypal[256];
	gfx_palette_get_phys(mypal);

	return {mypal[idx].r, mypal[idx].g, mypal[idx].b};
}
