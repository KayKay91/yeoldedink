...
/* Status animations */
int flife;
static int fstrength, fdefense, fmagic, last_magic_draw,
		/* flife, */ flifemax, fgold, fexp, fraise;
...
static void draw_num(int mseq, char* nums, int mx, int my) {
	...
	for (unsigned int i = 0; i < strlen(nums); i++) {
		...
		if (rnum == 11 || mseq == SEQ_EXPLEVEL_NUMS) {
			...
			SDL_Rect dst = {mx + length, my};
			IOGFX_background->blitNoColorKey(GFX_k[seq[mseq].frame[rnum]].k,
											NULL, &dst);
		}
...
//ye: changed these to display the thousand column
void draw_strength() {
	char final[3 + 2];
...
void draw_gold() {
	char final[6 + 1];
	snprintf(final, sizeof(final), (fgold < 100000) ? "%05d" : "%06d", fgold);
	...
	while (1) {
		...
		if (cur > life) {
			...
			if (rem != 0) {
				...
				dst.x = curx;
				dst.y = cury;
				IOGFX_background->blitNoColorKey(
						GFX_k[seq[seqman].frame[rnum]].k, &src, &dst);
			}
			//are we done?
			return;
		}
		...
		if ((cur / 10) * 10 == cur) {
			...
			dst.x = curx;
			dst.y = cury;
			IOGFX_background->blitNoColorKey(GFX_k[seq[seqman].frame[rnum]].k,
											NULL, &dst);
...
void draw_icons() {
	if (*pcur_weapon >= 1 && *pcur_weapon <= NB_ITEMS &&
		play.item[*pcur_weapon].active) {
		check_seq_status(play.item[*pcur_weapon].seq);
		SDL_Rect dst = {557, 413};
		IOGFX_background->blit(
				GFX_k[seq[play.item[*pcur_weapon].seq]
							.frame[play.item[*pcur_weapon].frame]]
						.k,
				NULL, &dst);
	}

	if (*pcur_magic >= 1 && *pcur_magic <= NB_MITEMS &&
		play.mitem[*pcur_magic].active) {
		check_seq_status(play.mitem[*pcur_magic].seq);
		SDL_Rect dst = {153, 413};
		IOGFX_background->blit(
				GFX_k[seq[play.mitem[*pcur_magic].seq]
							.frame[play.mitem[*pcur_magic].frame]]
						.k,
				NULL, &dst);
	}
}
...
void draw_mgauge_ver1(int percent, int mx, int my, int mseq, int mframe) {
	...
	dst.x = mx;
	dst.y = my;
	IOGFX_background->blitNoColorKey(GFX_k[seq[mseq].frame[mframe]].k, &src,
									&dst);
}
...
void draw_mgauge(int percent) {
	...
	if (percent > 0)
		draw_mgauge_ver1(percent, 149, 411, mseq, bary);
	percent -= 25;
	if (percent > 0)
		draw_mgauge_hor1(percent, 149, 409, mseq, barx);
	percent -= 25;
	if (percent > 0)
		draw_mgauge_ver2(percent, 215, 411, mseq, bary);
	percent -= 25;
	if (percent > 0)
		draw_mgauge_hor2(percent, 149, 466, mseq, barx);
}
...
void draw_status_all(void) {
	{
		...
		IOGFX_background->blitNoColorKey(GFX_k[seq[180].frame[3]].k, &src,
										&dst);
	}
	{
		SDL_Rect src = {0, 0, 20, 400}, dst1 = {0, 0}, dst2 = {620, 0};
		IOGFX_background->blitNoColorKey(GFX_k[seq[180].frame[1]].k, &src,
										&dst1);
		IOGFX_background->blitNoColorKey(GFX_k[seq[180].frame[2]].k, &src,
										&dst2);
	}
...
void update_status_all() {
	bool drawexp = false;
	int next = next_raise();
	auto goldlerp = tweeny::from(fgold).to(*pgold).via(easing::linear).during(1000);
	auto maglerp = tweeny::from(fmagic).to(*pmagic).via(easing::linear).during(1000);
	auto strlerp = tweeny::from(fstrength).to(*pstrength).via(easing::linear).during(1000);
	auto deflerp = tweeny::from(fdefense).to(*pdefense).via(easing::linear).during(1000);
	...
	if (next != fraise) {
		...
		drawexp = true;
		sfx_play_status_exp_tick();
	}

	if (*pexp != fexp &&
		((game_choice.active == 0 && show_inventory == 0 &&
		spr[1].freeze == 0) ||
		fexp + 10 < fraise)) {
		...
		drawexp = true;
		sfx_play_status_exp_count();

		if (fexp >= fraise) {
			*pexp -= next;
			fexp = 0;

			int script = scripting_load_script("lraise", 1, /*false*/ 0);
	...
	if (fstrength != *pstrength) {
		if (*pstrength - fstrength > 5 && dbg.statlerp) {
			fstrength = strlerp.step(0.2f);
		}
		else {
		if (fstrength < *pstrength)
			fstrength++;
		...
		sfx_play_status_stats();
		draw_strength();
	}

	if (fdefense != *pdefense) {
		if (*pdefense - fdefense > 5 && dbg.statlerp) {
			fdefense = deflerp.step(0.2f);
		}
		else {
		if (fdefense < *pdefense)
			fdefense++;
	...
	if (fmagic != *pmagic) {
		if (*pmagic - fmagic > 5 && dbg.statlerp) {
			fmagic = maglerp.step(0.2f);
		}
		else {
		if (fmagic < *pmagic)
			fmagic++;
	...
	if (flife < 1) {
		int script = scripting_load_script("dinfo", 1000, /*false*/ 0);
