...
extern bool callbacksystem_new;
...
struct script_engine
{
   ...
   */
  int (*load_script)(char *path, int script);
   ...
   */
  int (*script_proc_exists)(int script, char *proc);
   ...
   */
  int (*run_script_proc)(int script, char *proc);
...
//ye: commented out some unused fields
struct call_back
{
  int owner;
  /*bool*/int active;
...
//new callback system, should give it better name
extern entt::registry callbackreg;
struct ball_cack
{
  int owner;
  bool active;
...
extern struct script_engine script_engines[3];

extern int var_exists(const char name[20], int scope);
extern int scripting_make_int(char name[80], int value, int scope);
...
extern int scripting_proc_exists(int script, char* proc);
extern int scripting_run_proc(int script, char* proc);
...
extern void scripting_process_callbacks(Uint64 now);
