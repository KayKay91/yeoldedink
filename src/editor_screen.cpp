...
int load_screen_to(const char* path, const int num,
	...
	FILE* f = NULL;
	int32_t holdme, lsize;
	f = paths_dmodfile_fopen(path, "rb");
	//ye:fallbackfiule
	if (!f)
		f = paths_fallbackfile_fopen(path, "rb");
	...
	for (i = 0; i < 101; i++) {
		...
		fread(screen->sprite[i].script, 52, 1, f);
		screen->sprite[i].script[52 - 1] = '\0'; // safety
		...
		// offset 200
		//Convert last 20 bytes to userdata
		fread(screen->sprite[i].userdata, 20, 1, f);
		screen->sprite[i].userdata[20 - 1] = '\0';
		//for (int j = 0; j < 5; j++)
		//	read_lsb_int(f);
	}
	// offset 30204
...
/**
 * Save screen number 'num' in the map. Only used by the editor. yeolde: and me
 */
void save_screen(const char* path, const int num) {
	...
	if (num > 0) {
		...
		for (i = 0; i < 101; i++) {
			...
			fwrite(cur_ed_screen.sprite[i].script, 52, 1, f);
